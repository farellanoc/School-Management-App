...
function registerClass()
{
    ...
    if (count($errors) == 0) {
        ...
        if (mysqli_num_rows($res_check) == 0) {
            ...
            $id_schedule = "SELECT MAX(id_schedule) as id FROM schedule";  
            $queryResult = mysqli_query($db, $id_schedule);
            $id_schedule = (int) $queryResult->fetch_assoc()['id'];
            $id_schedule += 1;

/* Como horario y asignatura tienen que estar conectados, siempre que se cree una asignatura
    crearemos un horario. Seleccionamos el último id de schedule y le sumamos uno, ya que será
    el nuevo id que cogerá cuando después de ejecutar el insert de class ejecute el insert de schedule
    En schedule seleccionamos el último id_class y lo añadimos, de esta forma están conectadas.
    PD: A la larga esto puede dar problemas de persistencia si 2 administradores estuvieran creando
...
*/
            ...
            $id_class = "SELECT MAX(id_class) as id FROM class";  
            $queryResult = mysqli_query($db, $id_class);
            $id_class = (int) $queryResult->fetch_assoc()['id'];
...
function registerEnrollment()
{
    ...
    if (empty($id)) {
        array_push($errors, "Se requiere un id de estudiante");
    }
...
// call the registerEnrollment() function if registerEnrollment_btn is clicked
if (isset($_POST['registerEnrollment_btn'])) {
    registerEnrollment();
}
